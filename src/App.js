...
import useOnClickOutside, { useWindowSize } from "Components/hooks";
...
function App() {
	...
	useEffect(() => {
		...
		if (params.adults) {
			setAdultsCount(Number(params.adults))
		}
	...
	const firstDateHref = firstDate
		? `${firstDate.getFullYear()}-${addZeroToDate(firstDate.getMonth() + 1)
		}-${addZeroToDate(firstDate.getDate())}`
		: "";
	const secondDateHref = secondDate
		? `${secondDate.getFullYear()}-${addZeroToDate(secondDate.getMonth() + 1)
	...
	const buttonClickHandle = () => {
		const event = new CustomEvent("requestPrices", {
			detail: {
				adults: adultsCount,
				children: children.length,
				date_in: firstDateHref,
				date_out: secondDateHref,
				babies: 0,
				'children-age': children.join(";")
			}
		})
	...
	return (
		<form method="GET" action="/hotels/" test="hotels" className="js-booking-form searchHotel">
			...
			<div className="searchHotel__body">
				<div className="searchHotel__body-item kp-calendar-marker" onClick={openCalendar}>
					...
					{isCalendarOpen && <KPDatePicker
						...
						isTwoMounthsShow={detectOrientation()}
					/>}
				</div>
				<div className="searchHotel__body-item kp-calendar-marker" onClick={openCalendar}>
			...
			{/* </div> */}
			<button className="searchHotel__body-btn" type="button" onClick={buttonClickHandle}> Подобрать </button>
		</form>
	);
}
...
const Guests = ({
	...
	return (
		<div className="kp-select-hotel__guests" ref={panelRef}>
			<Counter title={"Взрослые"} minCount={1} onChange={changeAdultsCount} defaultValue={adultsCount} />
			<Select
